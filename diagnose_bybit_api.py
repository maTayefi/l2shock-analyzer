    ...
    try:
        ...
        if resp.status_code == 200:
            if "BTCUSDT" in resp.text:
                print("✅ SUCCESS: 'BTCUSDT' is confirmed available for Bybit orderbooks.")
            else:
                print("⚠️ WARNING: 'BTCUSDT' NOT found in response. Check if it's named 'BTCUSD' or similar.")
...
def test_file_access(client, label, file_path):
    ...
    try:
        ...
        with client.stream("GET", url, params=params, headers=headers, timeout=TIMEOUT, follow_redirects=True) as r:
            ...
            if r.status_code in (200, 206):
                ...
                if chunk[:4] == b'PAR1':
                    ...
                    print("⚠️ WARNING: Missing Parquet magic bytes. Might be an error page or XML.")
            else:
                print(f"❌ FAILED: Body snippet: {chunk.decode('utf-8', errors='ignore')[:200]}")
    except Exception as e:
        print(f"❌ ERROR: {type(e).__name__}: {e}")
