...
$RandomVersions = 1          # 1 = one random order, 3 = three random orders
$RandomSeed = $null          # set to a number like 12345 for repeatable output

$ExporterArgs = @(
    "--root", $AiCopyRoot,
    "--small-chunks",
    "--full-versions", $RandomVersions
)

if ($null -ne $RandomSeed) {
    $ExporterArgs += @("--seed", $RandomSeed)
}

& $PythonExe $CopiedExporterPath @ExporterArgs
